...
<body>
    <div class="admin-sidebar">
        ...
        $is_active = ($current_file == 'manage_officers.php'); // Corrected to match this file's name
    ...
    <div id="page-content-wrapper">
        <div class="container-fluid pt-4">
            ...
            <?php if ($action == 'list'): ?>
                <a href="manage_officers.php?action=add" class="btn btn-primary mb-4"><i class="fas fa-user-plus"></i> Add New Officer</a>
                ...
                if ($result && $result->num_rows > 0): ?>
                    <form method="POST" onsubmit="return confirm('Are you sure you want to delete the selected officers? (This action is irreversible)')">
                            ...
                            <table class="table table-striped table-hover align-middle">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="manage_officers.php?action=edit&id=<?php echo $row['id']; ?>" class="btn btn-sm btn-warning" title="Edit"><i class="fas fa-edit"></i></a>
                                            <a href="manage_officers.php?action=delete&id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this officer?')" class="btn btn-sm btn-danger" title="Delete"><i class="fas fa-trash"></i></a>
                                        </td>
                                    </tr>
                                    <?php endwhile; ?>
                                </tbody>
                        ...
                        <button type="submit" class="btn btn-danger mt-3" id="delete_selected_btn" disabled><i class="fas fa-trash-alt"></i> Delete Selected Officers</button>
                    </form>
                <?php else: ?>
                    <div class="alert alert-info">No club officers found.</div>
                <?php endif; ?>

            <?php elseif ($action == 'add' || $action == 'edit'): ?>
                <h2><?php echo $form_title; ?></h2>
                <hr>
                <form action="manage_officers.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="name" name="name" value="<?php echo htmlspecialchars($officer_data['name'] ?? ''); ?>" required>
                            </div>
                            ...
                            <div class="mb-3">
                                <label for="full_title" class="form-label">Full Title/Course/Section</label>
                                <select class="form-select" id="full_title" name="full_title" required>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="sort_order" name="sort_order" value="<?php echo htmlspecialchars($officer_data['sort_order'] ?? '0'); ?>" required>
                                <small class="form-text text-muted">Lower number means higher on the list/chart (0 for President, higher for VPs, etc.)</small>
                            </div>
                        </div>

                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="motto" class="form-label">Motto (Optional)</label>
                                <input type="text" class="form-control" id="motto" name="motto" value="<?php echo htmlspecialchars($officer_data['motto'] ?? ''); ?>">
                            </div>
                            
                            <div class="mb-3">
                                <label for="bio_content" class="form-label">Biography/Description</label>
                                <textarea class="form-control" id="bio_content" name="bio_content" rows="4"><?php echo htmlspecialchars($officer_data['bio_content'] ?? ''); ?></textarea>
                            </div>

                            <div class="mb-3">
                                <label for="email" class="form-label">Email</label>
                                <input type="email" class="form-control" id="email" name="email" value="<?php echo htmlspecialchars($officer_data['email'] ?? ''); ?>">
                            </div>
                            
                            <div class="mb-3">
                                <label for="linkedin" class="form-label">LinkedIn URL (Optional)</label>
                                <input type="url" class="form-control" id="linkedin" name="linkedin" value="<?php echo htmlspecialchars($officer_data['linkedin'] ?? ''); ?>">
                            </div>
                            
                            <div class="mb-3">
                                <label for="twitter" class="form-label">Twitter/X URL (Optional)</label>
                                <input type="url" class="form-control" id="twitter" name="twitter" value="<?php echo htmlspecialchars($officer_data['twitter'] ?? ''); ?>">
                            </div>
                        </div>
                    </div>
                    
                    <div class="mb-3">
                        <label for="image_path" class="form-label">Officer Profile Image Upload</label>
                        <input type="file" class="form-control" id="image_path" name="image_path" accept="image/*" <?php echo ($action == 'add' && empty($officer_data['image_path'])) ? 'required' : ''; ?>>
                        <?php if (!empty($officer_data['image_path'])): ?>
                            <div class="mt-2">
                                Current Image: <br>
                                <img src="../<?php echo htmlspecialchars($officer_data['image_path']); ?>" alt="Current Officer Image" style="width:100px; height: auto; border: 1px solid #ccc; border-radius: 50%;">
                            </div>
                            <input type="hidden" name="current_image" value="<?php echo htmlspecialchars($officer_data['image_path']); ?>">
                        <?php endif; ?>
                        <small class="form-text text-muted">A clear, square/circular portrait photo is recommended.</small>
                    </div>

                    <button type="submit" class="btn btn-success me-2"><i class="fas fa-save"></i> Save Officer</button>

...
if ($action == 'delete' && $item_id > 0) {
    ...
    if ($stmt_delete) {
        ...
        if ($stmt_delete->execute()) {
            ...
            if (reindex_project_ids($conn)) {
                $_SESSION['message'] = "Project (ID: $item_id) deleted and IDs re-indexed successfully (e.g., 3 becomes 2).";
                $_SESSION['msg_type'] = "success";
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $conn) {
    ...
    if (isset($_POST['bulk_action'], $_POST['selected_items']) && $_POST['bulk_action'] === 'delete_selected' && is_array($_POST['selected_items'])) {
        ...
        if (!empty($item_ids_to_delete)) {
            ...
            $images_to_delete = [];
            $sql_select_images = "SELECT image_path FROM projects WHERE project_id IN ($placeholders)";
    ...
    // --- Add/Update Single Project (No change needed here for re-indexing, as it doesn't affect sequential IDs) ---

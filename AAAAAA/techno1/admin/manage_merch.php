...
session_start();
// --- ADDED: Set Timezone for accurate logging timestamps ---
...
// --- ADDED: Activity Logging Function ---
// Assumes $_SESSION['admin_username'] is set upon login.
$admin_username = $_SESSION['admin_username'] ?? 'System'; 

function log_activity($conn, $username, $summary) {
    // The `timestamp` column in `admin_activity_log` is assumed to use MySQL's CURRENT_TIMESTAMP
    // The PHP time zone set above ensures the log entry time is accurate relative to the MySQL server time.
...
function reindex_merch_ids($conn) {
    ...
    if (!empty($all_merch_data)) {
        ...
        // Define types for binding (7 total): i(id), s(name), s(desc), s(cat), d(price), i(stock), s(url)
        $types = "isssdis"; 
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            if (reindex_merch_ids($conn)) {
                ...
                // --- ADDED LOGGING ---
                log_activity($conn, $admin_username, "Deleted merch item ID **$item_id** and performed ID re-indexing.");
            } else {
...
if ($action == 'reset_ids') {
    if (reindex_merch_ids($conn)) {
        ...
        $_SESSION['msg_type'] = "success";
        // --- ADDED LOGGING ---
        log_activity($conn, $admin_username, "Performed **full ID reset** on Merchandise table.");
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $conn) {
            ...
            if ($stmt_delete) {
                ...
                if ($stmt_delete->execute()) {
                    ...
                    if (reindex_merch_ids($conn)) {
                        ...
                        $_SESSION['message'] = "$deleted_count item(s) and their images deleted and IDs re-indexed successfully.";
                        $_SESSION['msg_type'] = "success";
                        // --- ADDED LOGGING ---
                        log_activity($conn, $admin_username, "Performed **bulk deletion** of $deleted_count merch items (IDs: " . implode(', ', $item_ids_to_delete) . ") and re-indexed IDs.");
                    } else {
    ...
    if (isset($_POST['action_type']) && in_array($_POST['action_type'], ['add_merch', 'update_merch'])) {
        ...
        $stock = (int)($_POST['stock'] ?? 0); // Handles stock of 0 correctly
        ...
        if ($is_update) {
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    ...
                    $_SESSION['msg_type'] = "success";
                    // --- ADDED LOGGING ---
                    log_activity($conn, $admin_username, "Updated merch item ID **$item_id**: **" . htmlspecialchars($name) . "**.");
                } else {
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    ...
                    $_SESSION['msg_type'] = "success";
                    // --- ADDED LOGGING ---
                    log_activity($conn, $admin_username, "Added new merch item ID **$new_merch_id**: **" . htmlspecialchars($name) . "**.");
                } else {
    ...
    <div id="page-content-wrapper" class="ml-250 p-8 w-[calc(100%-250px)] min-h-screen box-border">
        <div class="container mx-auto">
            ...
            <?php if ($action == 'list'): ?>
                ...
                <?php 
                // List view SELECT query (7 columns). This already selects all items, including stock = 0.

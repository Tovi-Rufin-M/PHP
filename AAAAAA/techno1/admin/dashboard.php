...
session_start();
// 🛠️ FIX: Set PHP timezone to Philippine Time (Asia/Manila) for accurate time calculations
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
    ...
    <div class="bg-color-card p-4 rounded-lg shadow-subtle-glow border border-color-bg-light">
        <div class="text-color-text-subtle text-sm font-semibold">Total Content Items</div>
        <div class="text-3xl font-extrabold text-color-text-base mt-1">420</div>
        <div class="text-xs text-green-400 mt-1"><i class="fas fa-arrow-up"></i> 12% Last Month</div>
    </div>
    
    <div class="bg-color-card p-4 rounded-lg shadow-subtle-glow border border-color-bg-light">
        <div class="text-color-text-subtle text-sm font-semibold">Published Posts</div>
        <div class="text-3xl font-extrabold text-color-primary mt-1">380</div>
        <div class="text-xs text-color-text-subtle mt-1">Currently Live</div>
    </div>
    
    <div class="bg-color-card p-4 rounded-lg shadow-subtle-glow border border-color-bg-light">
        <div class="text-color-text-subtle text-sm font-semibold">Items Pending Review</div>
        <div class="text-3xl font-extrabold text-yellow-400 mt-1">15</div>
        <div class="text-xs text-red-400 mt-1"><i class="fas fa-exclamation-triangle"></i> Action Required</div>
    </div>
    
    <div class="bg-color-card p-4 rounded-lg shadow-subtle-glow border border-color-bg-light">
        <div class="text-color-text-subtle text-sm font-semibold">Active Categories</div>
        <div class="text-3xl font-extrabold text-purple-400 mt-1">5</div>
        <div class="text-xs text-color-text-subtle mt-1">Manage Settings</div>
    </div>
...
<div class="mt-8 bg-bg-secondary p-6 rounded-xl shadow-xl border border-border-col">
    <div class="flex justify-between items-center mb-4">
        ...
        <button id="clearLogBtn" 
                class="text-sm bg-danger-red text-white px-3 py-1 rounded hover:bg-red-600 transition">
...
document.getElementById('clearLogBtn').addEventListener('click', function() {
    if (confirm('⚠️ Are you sure you want to clear ALL admin activity logs? This action cannot be undone.')) {
        
        // Disable button and show loading state
        this.disabled = true;
        this.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i> Clearing...';
        
        fetch('includes/clear_activity.php', {
        ...
        .then(data => {
            if (data.success) {
                alert(data.message);
                // Reload the page to show the cleared log
        ...
        .catch(error => {
            ...
            alert('An error occurred during the log clearing process.');
        })
        .finally(() => {
            // Re-enable button on error/completion if reload didn't happen
            document.getElementById('clearLogBtn').disabled = false;
            document.getElementById('clearLogBtn').innerHTML = '<i class="fas fa-trash-alt mr-1"></i> Clear Log';
        });
    }
});
</script>

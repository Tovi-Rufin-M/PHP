<?php
// technowatch/admin/manage_events_news.php - Full CRUD Management View (with Pinning Feature)
...
// --- UPDATED: Activity Logging Setup for robustness ---
$admin_username = $_SESSION['admin_username'] ?? 'System';

function log_activity($conn, $username, $summary) {
    // Explicitly generate the timestamp using the configured 'Asia/Manila' timezone
    $timestamp = date('Y-m-d H:i:s');

    // Updated query to explicitly include the timestamp column in the insert
    $stmt = $conn->prepare("INSERT INTO admin_activity_log (username, summary, timestamp) VALUES (?, ?, ?)");
    if ($stmt) {
        $stmt->bind_param("sss", $username, $summary, $timestamp);
        if (!$stmt->execute()) {
            // Added error logging for execution failure (for development/debugging)
            error_log("Activity Log Execution Error: " . $stmt->error);
        }
        $stmt->close();
    } else {
...
// =========================================================================
// NEW: FEATURED/PINNING TOGGLE LOGIC
// =========================================================================
if (isset($_GET['action']) && $_GET['action'] === 'toggle_feature' && isset($_GET['id'])) {
    $id = intval($_GET['id']);

    // Get current status and title
    $stmt = $conn->prepare("SELECT is_featured, title FROM events_news WHERE item_id = ?");
    ...
    if ($item) {
        ...
        $new_status = $item['is_featured'] == 0 ? 1 : 0;
        $status_text = $new_status == 1 ? 'featured/pinned' : 'un-featured/unpinned';

        // Update the status
        $update_stmt = $conn->prepare("UPDATE events_news SET is_featured = ? WHERE item_id = ?");
        $update_stmt->bind_param("ii", $new_status, $id);

        if ($update_stmt->execute()) {
            log_activity($conn, $admin_username, "Toggled feature status of item '{$item['title']}' to '{$status_text}' (ID: {$id})");
            $_SESSION['message'] = "Item '{$item['title']}' has been successfully {$status_text} for homepage preview.";
            $_SESSION['msg_type'] = "success";
        } else {
            $_SESSION['message'] = "Error toggling feature status: " . $update_stmt->error;
...
$item_data = [
    ...
    'location' => '', 'event_time' => '', 'is_featured' => 0 // ADDED: is_featured default
];
            ...
            if ($fetch_stmt) {
                ...
                if (call_user_func_array([$fetch_stmt, 'bind_param'], array_by_ref($bind_params_fetch))) {
                    ...
                    foreach ($images_to_delete as $image_path_db) {
                        // Adjusted path resolution for robust file deletion
                        $full_file_path = $admin_dir . '/../' . ltrim($image_path_db, '/');
                    ...
                    if ($delete_stmt) {
                        ...
                        if (call_user_func_array([$delete_stmt, 'bind_param'], array_by_ref($bind_params_delete))) {
                            if ($delete_stmt->execute()) {
                                ...
                                $ids_list = implode(', ', $item_ids_to_delete);
                                log_activity($conn, $admin_username, "Performed **bulk deletion** of $deleted_count event/news item(s) (IDs: $ids_list). $files_deleted_count image(s) removed.");
                                // --- END LOGGING ---
...
if ($action == 'edit' && $item_id > 0) {
    ...
    if ($result->num_rows === 1) {
        ...
        $fetched_data['event_date'] = !empty($fetched_data['event_date']) ? date('Y-m-d', strtotime($fetched_data['event_date'])) : '';
        $fetched_data['event_time'] = !empty($fetched_data['event_time']) && strtotime($fetched_data['event_time']) !== false ? date('H:i', strtotime($fetched_data['event_time'])) : '';
    ...
    <div id="page-content-wrapper" class="ml-250 p-8 w-[calc(100%-250px)] min-h-screen box-border">
        <div class="container mx-auto">
            ...
            <?php if ($action == 'list'): ?>
                ...
                <?php
                // MODIFIED QUERY: Prioritize featured items (is_featured DESC), then by creation date.
                $query = "SELECT * FROM events_news ORDER BY is_featured DESC, created_at DESC";
                                ...
                                <thead class="text-xs uppercase bg-dark-input text-white border-b border-dark-border">
                                    <tr>
                                        ...
                                        <th scope="col" class="py-3 px-6">Published</th>
                                                                                                          <th scope="col" class="py-3 px-6">Featured</th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-b border-dark-border even:bg-dark-card odd:bg-dark-card/70 hover:bg-dark-input transition duration-150">
                                        ...
                                        <td class="py-4 px-6">
                                            ...
                                            $link = "manage_events_news.php?action=toggle_feature&id=" . $row['item_id'];
                                            $button_class = $is_featured ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700';
                                            $button_text = $is_featured ? '<i class="fas fa-thumbtack mr-1"></i> Pinned' : '<i class="fas fa-thumbtack mr-1"></i> Pin';
                                            $title_text = $is_featured ? 'Unpin from Homepage' : 'Pin to Homepage (Preview Priority)';
                                            ?>
                                            <a href="<?php echo $link; ?>"
                                               class="inline-flex items-center text-xs font-semibold px-2.5 py-1 rounded-full text-white transition-colors duration-200 <?php echo $button_class; ?>"
            ...
            <?php elseif ($action == 'add' || $action == 'edit'): ?>
                ...
                <form action="actions.php" method="POST" enctype="multipart/form-data" class="bg-dark-card p-8 rounded-xl shadow-2xl space-y-6 border border-dark-border">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" class="form-checkbox w-5 h-5 accent-primary-indigo cursor-pointer bg-dark-input border-dark-border rounded" id="is_featured" name="is_featured" value="1" <?php if(($item_data['is_featured'] ?? 0)) echo 'checked'; ?>>
                        <label for="is_featured" class="ml-3 text-text-light font-medium"><i class="fas fa-thumbtack mr-1"></i> Feature/Pin to Homepage Preview</label>
                    </div>

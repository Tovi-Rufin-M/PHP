...
function log_activity($conn, $username, $summary) {
    // Assumes an 'admin_activity_log' table exists with columns: id, username, summary, timestamp
            ...
            if ($delete_stmt) {
                ...
                if (call_user_func_array([$delete_stmt, 'bind_param'], array_by_ref($bind_params_delete))) {
                    if ($delete_stmt->execute()) {
                        ...
                        log_activity($conn, $admin_username, "Performed **bulk deletion** of $deleted_count job posting(s) (IDs: $ids_list).");
                        // --- END ADDED LOGGING ---

...
<i class="fas fa-mobile-alt"></i> IoT
</button>
...
<option value="all-tags" selected>Status</option>
...
// MODIFIED: Pass null for card override when clicking
$projectCards.off('click').on('click', (e) => openProjectModal(e, null)); 
applyFilters(true);
console.log('Projects list refreshed successfully.');
// ADDED: Check URL for ID after grid refresh
openModalFromURL();
},
...
// ADDED: Function to handle opening the modal when navigating from another page via URL
function openModalFromURL() {
    const urlParams = new URLSearchParams(window.location.search);
    const projectId = urlParams.get('id');

    if (projectId) {
        // Find the project card that matches the ID
        const $targetCard = $projectCards.filter(`[data-id="${projectId}"]`);

        if ($targetCard.length > 0) {
            // Manually trigger the modal function using the card data
            // We pass a dummy event object since openProjectModal expects it.
            openProjectModal({ preventDefault: () => {} }, $targetCard);

            // Optional: Remove the ID from the URL bar cleanly
...
// MODIFIED: Accept optional card for external calls (e.g., from openModalFromURL)
function openProjectModal(e, $cardOverride = null) {
e.preventDefault();
const $card = $cardOverride || $(this); // Use override or the clicked element
...
// FIX APPLIED HERE: Prevents default button action (which causes navigation)
